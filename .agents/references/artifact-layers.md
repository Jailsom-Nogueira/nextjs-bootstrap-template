...
| `.plans/`             | active implementation plans             | standalone HTML/CSS plans: slices, files touched, verification | permanent product truth after completion        |
| `.plans/archived/`    | completed/superseded plans              | historical standalone HTML plan records                        | active work                                     |
...
- **Plan format** = standalone `.html` with embedded CSS, using `.plans/templates/plan.html` and the self-contained Plan Document System. This is only for implementation plans; specs, docs, ADRs, and runbooks remain Markdown unless they are intentionally shipped as human-facing rich artifacts.
...
- Creating or editing a plan → read `.plans/templates/plan.html`.
