...
- Use the `Read` tool on `AGENTS.md`, then load specific `.agents/rules/*.md` for the task at hand. Do not load all rules eagerly.
- Prefer `Edit` over `Write` for targeted changes — preserves history cleaner.
- After editing, run `npm run check` via `Bash` before declaring done.
