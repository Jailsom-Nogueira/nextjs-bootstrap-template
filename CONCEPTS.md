...
4. [Conventional Commits + Carevia-style CHANGELOG](#4-conventional-commits--carevia-style-changelog)
...
(format:check → lint → typecheck → test → build), stops at the **first** failing
...
## 4. Conventional Commits + Carevia-style CHANGELOG
...
`scripts/generate-changelog.ts` (the "Carevia-style" pattern: bumps patch
...
generator something deterministic to parse. The Carevia approach was
proven in production at a previous project — it's simpler than
`changesets` for solo or small-team projects and creates a real
human-readable log of every push.
