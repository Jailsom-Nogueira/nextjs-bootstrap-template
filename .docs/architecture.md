...
| Runtime        | Node 22 + edge (middleware)                      | `.nvmrc` pins                               |
...
| Data / Auth    | Supabase (`@supabase/ssr`)                       | 4-client split                              |
...
flowchart LR
  Browser((Browser))
  subgraph Vercel
    MW[middleware.ts<br/>session refresh + locale + admin gate]
...
2. **`src/middleware.ts`** — runs at the edge for every request: refreshes Supabase session cookies, enforces locale routing, and redirects unauthenticated users away from gated routes. For the admin tree it also calls `isAdmin()` so a bad cookie can't reach the layout.
...
- **Vercel** runs Next.js (Server Components, Server Actions, Route Handlers), the middleware on the edge, and Vercel Cron when configured.
